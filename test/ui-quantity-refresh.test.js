...
test('property panel refreshes quantity summary through a model revision cache', async () => {
  ...
  // The heavy full-model JSON signature was replaced by a lightweight key built
  // from the monotonic state revision (bumped on every mutation) and language.
  assert.match(uiSource, /const key = `\$\{this\.state\.revision\}\|\$\{getLang\(\)\}`;/);
  assert.doesNotMatch(uiSource, /_quantitySummaryStateKey/);
  assert.doesNotMatch(uiSource, /levels:\s*this\.state\.levels/);
